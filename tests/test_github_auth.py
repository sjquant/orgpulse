...
class TestGitHubAuthService:
    def test_prefers_github_token_from_config(
    ...
    def test_rejects_missing_auth_sources(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        # When
        with pytest.raises(
            AuthResolutionError, match="GitHub authentication is required"
        ):
    ...
    def test_rejects_inaccessible_target_organization(self) -> None:
        ...
        # When
        with pytest.raises(
            OrgTargetingError, match="Target organization 'acme' was not found"
        ):
    ...
    def test_rejects_invalid_resolved_token(self) -> None:
        ...
        # When
        with pytest.raises(
            AuthResolutionError, match="resolved credentials were rejected"
        ):
            service.validate_access(config)

        # Then

    def test_surfaces_non_auth_viewer_lookup_failures_as_github_api_errors(
        self,
    ) -> None:
...
class FakeGithubAuthClient:
    def __init__(self) -> None:
        self.get_user_mock = Mock(return_value=SimpleNamespace(login="maintainer"))
        self.get_organization_mock = Mock(return_value=SimpleNamespace(login="acme"))

    def get_user(self) -> SimpleNamespace:
        return self.get_user_mock()

    def get_organization(self, org: str) -> SimpleNamespace:
        return self.get_organization_mock(org)
